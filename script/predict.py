...
def test_divide_to_batch(batch_number, sample_height, sample_width, sample_y_limit, sample_x_limit, sample, sample_stride, sample_size):
    ...
    for y in range(0, sample_y_limit, sample_stride):
        for x in range(0, sample_x_limit, sample_stride):
            ...
            sample_batch.append(sample[y: y + sample_size, x: x + sample_size])
...
def create_minibatch(test_fn, batch_number, sample_height, sample_width, sample_y_limit, sample_x_limit, sample_stride, sample_size, out_dir_compression):
    ...
    sample = cv.imread('{}/{}_sample.jpg'.format(out_dir_compression, image_fn))
    sample_batch = test_divide_to_batch(batch_number, sample_height, sample_width, sample_y_limit, sample_x_limit, sample, sample_stride, sample_size)
...
def run(result_dir, predict_epoch,
        ...
        image_size, image_stride, image_height, image_width, crop):
    if crop > 0:
    ...
    for test_fn in test_fns:
        ...
        sample, sample_batch, image_fn = create_minibatch(test_fn,
                                                        batch_number, sample_height, sample_width, sample_y_limit, sample_x_limit,
                                                        sample_stride, sample_size, out_dir_compression)
        ...
        save_image(sample, image_fn, image, out_dir_compression, out_dir_decompression)
